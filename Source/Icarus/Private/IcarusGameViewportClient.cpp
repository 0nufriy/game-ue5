...
#include "IcarusGameViewportClient.h"

void UIcarusGameViewportClient::Init(struct FWorldContext& WorldContext, UGameInstance* OwningGameInstance, bool bCreateNewAudioDevice)
{
...
bool UIcarusGameViewportClient::OverrideInputAxisHandler(FInputKeyEventArgs& EventArgs, float& Delta, float& DeltaTime, int32& NumSamples)
{
...
bool UIcarusGameViewportClient::OverrideInputKey(FInputKeyEventArgs& EventArgs)
{
    if (EventArgs.IsGamepad()) {
        if (const auto GI = GetGameInstance()) {
            const int32 NumLocalPlayers = GI->GetNumLocalPlayers();
